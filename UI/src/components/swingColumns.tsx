...
const BASE_COLUMNS: Record<SwingTabId, GridColDef<SwingStock>[]> = {
  dashboard: [
    ...
    extraField('ltp', 'LTP', (v) => `₹${Number(v).toFixed(2)}`),
    extraField('change_pct', 'Change %', (v) => `${v}%`),
    extraField('return_5d_pct', '5D Return', (v) => `${v}%`),
    extraField('return_10d_pct', '10D Return', (v) => `${v}%`),
    extraField('return_20d_pct', '20D Return', (v) => `${v}%`),
    ...
    extraField('rsi', 'RSI'),
    extraField('adx', 'ADX'),
    extraField('atr', 'ATR', (v) => `₹${Number(v).toFixed(2)}`),
    extraField('ema_20', 'EMA20', (v) => `₹${Number(v).toFixed(2)}`),
    extraField('ema_50', 'EMA50', (v) => `₹${Number(v).toFixed(2)}`),
    extraField('ema_200', 'EMA200', (v) => `₹${Number(v).toFixed(2)}`),
    extraField('dist_52w_high_pct', 'From 52W H', (v) => `${v}%`),
    extraField('breakout', 'Breakout', (v) => (v ? 'Yes' : 'No')),
    extraField('relative_strength_pct', 'Rel Strength', (v) => `${v}%`),
    extraField('sector_rank', 'Sector Rank'),
    extraField('momentum_score', 'Momentum Score'),
    extraField('entry_price', 'Entry', (v) => `₹${Number(v).toFixed(2)}`),
    extraField('stop_loss', 'Stop Loss', (v) => `₹${Number(v).toFixed(2)}`),
    extraField('target_1', 'Target 1', (v) => `₹${Number(v).toFixed(2)}`),
    extraField('target_2', 'Target 2', (v) => `₹${Number(v).toFixed(2)}`),
    extraField('risk_reward', 'Risk Reward', (v) => `${v}:1`),
    extraField('confidence', 'Confidence', (v) => `${v}%`),
    extraField('ai_remarks', 'AI Remarks', (v) => String(v).slice(0, 100)),
  ],
  stock_score: [
    extraField('rank', 'Rank'),
